  ...
  return (
    <div className="w-full space-y-[1.5rem]">
      ...
      <div className="hidden md:block w-full overflow-x-auto custom-scrollbar">
        <table className="w-full text-left border-separate border-spacing-y-[0.5rem]">
          <thead>
            <tr className="text-[0.6875rem] font-black text-textMain/50 uppercase tracking-[0.15em]">
              <th className="px-[1.5rem] py-[1rem]">Invoice Reference</th>
              <th className="px-[1.5rem] py-[1rem]">Sales Representative</th>
              <th className="px-[1.5rem] py-[1rem]">Customer</th>
              <th className="px-[1.5rem] py-[1rem]">Manifested Products & Variants</th>
              <th className="px-[1.5rem] py-[1rem] text-right">Total (LKR)</th>
            </tr>
          </thead>
          <tbody>
            {currentOrders.map((order) => {
              ...
              const orderValue = targetItems.reduce((sum, item) => sum + (parseFloat(item.price) * item.quantity), 0) || 0;

              return (
                <tr key={order.order_id} className="group transition-all text-[0.8125rem]">
                  
                  {/* Reference ID */}
                  <td className="px-[1.5rem] py-[1.25rem] bg-background border-y border-l border-border rounded-l-[1rem] font-mono font-bold text-textMain">
                    #{order.order_id.substring(0, 8).toUpperCase()}
                    <p className="text-[0.625rem] text-textMain/40 font-sans font-medium mt-[0.25rem]">
                      {new Date(order.created_at || order.createdAt).toLocaleDateString('en-GB')}
                    </p>
                  </td>

                  {/* Rep */}
                  <td className="px-[1.5rem] py-[1.25rem] bg-background border-y border-border">
                    <p className="font-bold text-textMain">{repName}</p>
                    <p className="text-[0.6875rem] text-textMain/50 font-medium">{repEmail}</p>
                  </td>

                  {/* Saloon Info */}
                  <td className="px-[1.5rem] py-[1.25rem] bg-background border-y border-border">
                    <p className="font-bold text-textMain">{order.customer?.saloon_name || 'Direct Order'}</p>
                    <p className="text-[0.6875rem] text-textMain/50 uppercase tracking-wider font-semibold">
                      📍 {order.customer?.district || order.district}
                    </p>
                  </td>

                  {/* Products & Variant List */}
                  <td className="px-[1.5rem] py-[1.25rem] bg-background border-y border-border max-w-[18rem]">
                    <div className="space-y-[0.375rem]">
                      {targetItems.map((item, idx) => (
                        <div key={idx} className="flex justify-between items-center bg-card/60 px-[0.5rem] py-[0.25rem] rounded-[0.5rem] border border-border/40">
                          <span className="font-medium text-textMain/80 truncate pr-[0.5rem]">
                  ...
                  {/* Total price value */}
                  <td className="px-[1.5rem] py-[1.25rem] bg-background border-y border-r border-border rounded-r-[1rem] text-right font-mono font-bold text-textMain text-[0.875rem]">
      ...
      <div className="block md:hidden space-y-[1rem]">
        {currentOrders.map((order) => {
          const targetItems = order.OrderItems || order.items || [];
          const repName = order.creator?.name || order.sales_rep?.name || 'Unknown Rep';
          const orderValue = targetItems.reduce((sum, item) => sum + (parseFloat(item.price) * item.quantity), 0) || 0;
          ...
          return (
            <div key={order.order_id} className="p-[1.25rem] bg-background border border-border rounded-[1.25rem] space-y-[1rem] transition-all">
              ...
              <div className="grid grid-cols-2 gap-[0.75rem] text-[0.75rem]">
                <div className="space-y-[0.125rem]">
                  ...
                  <p className="font-bold text-textMain truncate">{repName}</p>
                </div>
                <div className="space-y-[0.125rem]">
                  <p className="text-[0.625rem] text-textMain/40 uppercase font-black tracking-wider flex items-center gap-[0.25rem]">
                    <MapPin size={10} /> Saloon / Client
                  </p>
                  <p className="font-bold text-textMain truncate">{order.customer?.saloon_name || 'Direct Order'}</p>
                </div>
              </div>
                ...
                <div className="space-y-[0.375rem]">
                  {targetItems.map((item, idx) => (
                    <div key={idx} className="flex justify-between items-center text-[0.75rem]">
                      <span className="text-textMain/80 font-medium truncate pr-[0.5rem]">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between pt-[1rem] border-t border-border/60 text-[0.8125rem]">
          {/* Information string */}
