                        ...
                        <div className="bg-card rounded-[2rem] border border-border overflow-hidden shadow-sm">
                            <table className="w-full text-left">
                                <thead className="bg-background border-b border-border">
                                    <tr>
                                        <th className="px-6 py-4 text-[10px] font-black uppercase text-textMain/50 tracking-widest">Rank</th>
                                        <th className="px-6 py-4 text-[10px] font-black uppercase text-textMain/50 tracking-widest">Sales Rep</th>
                                        <th className="px-6 py-4 text-[10px] font-black uppercase text-textMain/50 tracking-widest text-right">Achieved Value</th>
                                        <th className="px-6 py-4 text-[10px] font-black uppercase text-textMain/50 tracking-widest text-center">Progress</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-border">
                                    {rankings.slice(3).map((rep, idx) => (
                                        <tr key={rep.user_id} onClick={() => navigate(`/profile/${rep.user_id}`)} className="hover:bg-primary/5 transition-colors cursor-pointer group">
                                            <td className="px-6 py-4 text-sm font-black text-textMain/40">#{idx + 4}</td>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    {rep.profile_image ? (
                                                        <img src={rep.profile_image} alt={rep.name} className="w-8 h-8 rounded-lg object-cover border border-border" />
                                                    ) : (
                                                        <div className="w-8 h-8 rounded-lg bg-primary/20 text-primary flex items-center justify-center text-[10px] font-black">{getInitials(rep.name)}</div>
                                                    )}
                                                    <span className="font-bold text-sm text-textMain group-hover:text-primary transition-colors">{rep.name}</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex flex-col items-end">
                                                    <div>
                                                        <span className="text-primary text-[10px] mr-1 font-bold">LKR</span>
                                                        <span className="font-black text-textMain">{Number(rep.achieved).toLocaleString()}</span>
                                                    </div>
                                                    <span className="text-[9px] text-textMain/40 font-bold uppercase mt-0.5 tracking-widest">Target: {Number(rep.target).toLocaleString()}</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col items-center">
                                                    <span className={`text-[10px] font-black mb-1 ${rep.achievementPercentage >= 100 ? 'text-emerald-500' : 'text-primary'}`}>
                                                        {rep.achievementPercentage.toFixed(1)}%
                                                    </span>
                                                    <div className="w-24 h-1.5 bg-background rounded-full overflow-hidden">
                                                        <div 
                                                            className={`h-full rounded-full ${rep.achievementPercentage >= 100 ? 'bg-emerald-500' : 'bg-primary'}`} 
