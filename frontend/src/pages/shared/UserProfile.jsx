...
const UserProfile = () => {
  ...
  if (!user)
    return (
      <div className="p-10 text-center font-bold italic text-primary tracking-widest">
        Initialising...
      </div>
    );
      ...
      <div className="flex flex-col md:flex-row items-center gap-8 mb-10 bg-card p-8 rounded-[2rem] shadow-sm border border-border">
        <div className="relative group">
          ...
          {isEditing && (
            <button
              ...
              className="absolute -bottom-2 -right-2 p-2 bg-black text-white rounded-xl shadow-lg hover:bg-primary transition-colors"
            >
              <Camera size={16} />
            </button>
          )}
          <input
            ...
            onChange={(e) => {
              ...
              if (file) {
                ...
                reader.onload = (ev) =>
                  setFormData({ ...formData, picture_url: ev.target.result });
        ...
        <div className="text-center md:text-left flex-1">
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-3 mt-2">
            ...
            <span
              className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase border ${user.is_active ? "bg-green-50 text-green-600" : "bg-red-50 text-red-600"}`}
            >
        ...
        {(isOwnProfile || isAdmin) && (
          <div className="flex flex-col sm:flex-row md:flex-col gap-2">
            ...
            {/* Security button only for own profile */}
            ...
            <section className="bg-card p-10 rounded-[2.5rem] shadow-sm border border-border h-full">
              <div className="flex items-center justify-between mb-8">
                ...
                {isAdmin && (
                  <button
                    ...
                    className={`text-[9px] font-black uppercase px-4 py-2 rounded-xl transition-all ${isEditingAreas ? "bg-black text-white" : "bg-background text-textMain/60"}`}
                  >
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {userAreas.length > 0 ? (
                  userAreas.map((area, idx) => (
                    ...
                    >
                      ...
                      {isAdmin && isEditingAreas && (
                        <button
                          onClick={() => handleRemoveArea(area.district_name)}
                  ...
                  <p className="text-xs font-bold text-textMain/40 italic">
                    No assigned areas.
                  </p>
                )}
              </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 max-h-[200px] overflow-y-auto pr-2 custom-scrollbar">
                    {districtsList
                      ...
                      .map((d) => (
                        <button
                          key={d}
                          onClick={() => handleAddArea(d)}
                          className="text-[9px] font-bold py-2 border border-border rounded-lg hover:border-primary hover:bg-primary/5 text-textMain/60"
                        >
                          + {d}
                        </button>
                      ))}
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="p-2 bg-background text-primary rounded-xl hover:bg-primary hover:text-black transition-all"
              >
                ...
                {loadingCustomers ? (
                  ...
                  customers.map((cust) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-5 bg-background border-y border-border text-[10px] font-bold text-textMain/70 uppercase">
                        {cust.district}
                      </td>
                      <td className="px-6 py-5 bg-background border-y border-border text-xs font-bold">
                        {cust.phone1 || cust.phone2 || "N/A"}
                      </td>
                      <td className="px-6 py-5 bg-background rounded-r-2xl border-y border-r border-border text-right">
                        <span className="text-[10px] font-black uppercase text-green-600 bg-green-500/10 px-3 py-1 rounded-lg border border-green-500/20">
                          Mapped
                        </span>
                      </td>
                    </tr>
                  ...
                  <tr>
                    <td
                      colSpan="4"
                      className="text-center py-10 text-textMain/40 font-bold uppercase text-xs italic"
                    >
                      No portfolio data.
                    </td>
                  </tr>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[1000] flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="bg-card w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl animate-in zoom-in">
            <div className="flex justify-between items-center mb-10">
              ...
              >
                <X size={24} className="text-textMain/60" />
              </button>
            </div>
            <div className="space-y-6">
              ...
              <button
                ...
                disabled={isUpdating}
                className="w-full py-4 bg-black text-white font-bold rounded-2xl hover:bg-primary transition-all flex items-center justify-center gap-2 text-sm"
              >
    ...
    <div className="space-y-2 text-left">
      ...
      <div className="relative">
        ...
        <button
          ...
          className="absolute right-4 top-1/2 -translate-y-1/2 text-textMain/60 hover:text-primary"
        >
